...
from database_setup import Base, CatalogItemType
...
def gconnect():
    ...
    if (result['user_id'] != gplus_id):
        response = make_response(json.dumps('Token user_id doesnt match given user'),401)
        response.headers['Content-Type'] = 'application/json'
        return response
...
def catalogMenuItem(category_id):
    catalogItems = session.query(CatalogItem).filter_by(catalog_id=category_id).all()
    catalog = session.query(Catalog).all()
...
@app.route('/categories/<int:category_id>/<int:item_id>/type')
def catalogMenuItemType(category_id,item_id):
    catalogItemTypes = session.query(CatalogItemType).filter_by(catalog_item_id=item_id).all()
    catalogItem = session.query(CatalogItem).filter_by(id=item_id).one()
    catalogItems = session.query(CatalogItem).filter_by(catalog_id=catalogItem.catalog_id).all()
    catalog = session.query(Catalog).all()
    print(item_id)
    catalog = session.query(Catalog).all()
    if (request.method == 'POST'):
        ...
        return jsonify(CatalogItemType=[i.serialize for i in catalogItemTypes])


# helper functions for users

def createUser(login_session):
    newUser = UserInfo(name=login_session['username'],email = login_session['email'], picture=login_session['picture'])
    session.add(newUser)
    session.commit()
    user = session.query(UserInfo).filter_by(email=login_session['email']).one()
    return user.id
